...
function setTalkingOnRadio(talking)
	...
	if radioTbl then
		...
		playerIdentity = playerIdentity.nome.." "..playerIdentity.sobrenome.." ["..user_id.."]"
		
		for player, _ in pairs(radioTbl) do
			-- TriggerClientEvent('pma-voice:setTalkingOnRadio', player, source, talking, playerIdentity, user_id)
			TriggerClientEvent('flaviin:setPlayerTalking', player, talking, playerIdentity, user_id)
			if player ~= source then
				TriggerClientEvent('pma-voice:setTalkingOnRadio', player, source, talking, playerIdentity)
...
RegisterServerEvent('pma-voice:toggleMutePlayer')
AddEventHandler('pma-voice:toggleMutePlayer', function(status)
	MumbleSetPlayerMuted(source, status or false)

	if status then
		TriggerClientEvent('Notify',source,'sucesso',"Você foi mutado.")
	else
		TriggerClientEvent('Notify',source,'sucesso',"Você foi desmutado.")
	end
end)