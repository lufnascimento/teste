...
function vRP.forceRegister(name,builder)
	local mbuilders = menu_builders[name]
	mbuilders = {}
	menu_builders[name] = mbuilders
	table.insert(mbuilders,builder)
...
function tvRP.openOrgMenu()
    ...
    if vRP.hasPermission(user_id, "perm.policia") or vRP.hasPermission(user_id, "perm.prf")  or vRP.hasPermission(user_id,"perm.policiacivil") or vRP.hasPermission(user_id,"perm.core") or vRP.hasPermission(user_id,"perm.pf") or vRP.hasPermissions(user_id, "perm.tatica") then
	...
	elseif vRP.hasPermission(user_id, "perm.bombeiro") then
		local menudata = vRP.buildMenu("unizk_menu", {player = source})
		menudata.name = "BOMBEIRO"
		vRP.openMenu(source,menudata)
    end
end