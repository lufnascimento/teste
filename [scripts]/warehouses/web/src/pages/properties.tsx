...
export function PropertiesPage() {
  const [properties, setAvailableProperties] = useState<AvailablePropertiesProps[]>([])
  const [search, setSearch] = useState<string>('')
  const navigate = useNavigate()

  useEffect(() => {
    fetch<AvailablePropertiesProps[]>('GET_PROPERTIES', null, [
      {
        id: 31,
            ...
            {filteredProperties.map(properties => (
              <div className="rounded-[5px] bg-white/[0.01] p-2.5 flex flex-col gap-2.5 max-w-[19.9375rem] w-full max-h-[12.5rem]">
                ...
                <div className="w-full flex justify-between items-center">
                  ...
                  <span onClick={() => navigate('/properties/'+properties.id)} className="h-[30px] px-3 py-1.5 flex gap-2 items-center group bg-white/[0.03] rounded-[5px]">
                    <i className="fa-light fa-pen text-white/45 cursor-pointer transition-all group-hover:!text-white/100"></i>
                  </span>
                </div>
              </div>
            ))}
